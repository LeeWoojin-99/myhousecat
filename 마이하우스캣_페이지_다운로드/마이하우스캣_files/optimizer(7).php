$(function(){
    ...
    var swiper3 = new Swiper('.sub_visual', {
        pagination: '.sub_visual .swiper-pagination',
        slidesPerView: 1,
        paginationClickable: true,
        spaceBetween: 0,
        loop: true,
...
jQuery1_12_4(function () {
	jQuery1_12_4("#pop1, #pop2").draggable();
});
/**
 * Swiper 3.4.1
...
$(function(){
    ...
    // title, alt remove
    var slide_imgLink = $(".main_top .swiper-slide a").length;
    if(slide_imgLink > 0){
        $(".main_top .swiper-slide a").attr("alt","");
        $(".main_top .swiper-slide img").attr("title","");
    } else {
        $(".main_top .swiper-slide img").attr("title","");
    }
    
    // img change background
    var mainBnnH = $(".main_top .swiper-slide img").height();
    $(".main_top .swiper-slide > a").each(function(){
    ...
    // title, alt remove
    var slide_imgLink02 = $(".bnn_full div a").length;
    if(slide_imgLink02 > 0){
        $(".bnn_full div a").attr("alt","");
        $(".bnn_full div img").attr("title","");
    } else {
        $(".bnn_full div img").attr("title","");
    }
    
    // img change background
    var mainBnnH02 = $(".bnn_full div img").height();
    $(".bnn_full div > a").each(function(){
        var imgSrc02 = $(this).children("img").attr("src");
        $(this).css({
            "height": mainBnnH02+"px",
            "background-image": "url("+imgSrc02+")"
        });
        $(this).find("img").css("visibility","hidden");
    });
...
$(function(){
    ...
    $(".myMenu").hover(function(){
        $(this).find("i").removeClass("xi-user-plus-o");
        $(this).find("i").addClass("xi-user-plus");
    }, function(){
        $(this).find("i").removeClass("xi-user-plus");
        $(this).find("i").addClass("xi-user-plus-o");
    });
    $(".myCart").hover(function(){
        $(this).find("i").removeClass("xi-cart-o");
        $(this).find("i").addClass("xi-cart");
    }, function(){
        $(this).find("i").removeClass("xi-cart");
        $(this).find("i").addClass("xi-cart-o");
    });
...
1. [공통] 한가지 로고만 사용하는 스킨의 경우 (스크롤 시 색상이 변경되지 않는 기본 타입)
 - 아래의 default_logo 값에 로고의 위치 및 파일명을 작성해줍니다.
 - 로고 이미지가 없는경우 비워 둡니다. (비워두는 경우 기본 텍스트로 상점명이 출력됩니다.)
 
 - 로고 이미지가 있는 경우
 var default_logo = "파일위치/파일명";

 ex) 적용 예시
 var default_logo = "/urban/images/layout/header/logo.png";

 - 로고 이미지가 없는 경우 (기본설정)
 var default_logo = "";


2. 두 가지의 로고가 사용되는 스킨의 경우 (스크롤 시 색상이 변하는 타입)
 - 위 1번의 기본 로고 default_logo 값을 설정 후 스크롤 시 변경되는 로고(경로/파일명) scroll_logo 값을 설정해주세요.
 - 로고 이미지가 있는 경우
 var scroll_logo = "파일위치/파일명";

 ex) 적용 예시
 var scroll_logo = "/urban/images/layout/header/logo_w.png";

 - 로고 이미지가 없는 경우 (기본설정)
 var scroll_logo = "";

※ 로고 설정 시 주의 사항
- 정확한 로고의 위치(경로)와 파일명을 입력해 주세요 (잘못입력 시 로고가 출력되지 않습니다)
- 문법에 맞게 설정해주세요. (큰따음표, 세미콜론 등)
- 로고가 없는 경우 비워두세요
...
/******************************************************************************
주의 : 아래 소스는 절대 수정 금지 !! 수정시 사이트에 오류가 발생할 수 있습니다. 
(로고 설정은 위 default_logo 또는 scroll_logo 값만 수정바랍니다.)
******************************************************************************/
if(default_logo == ""){
    $(".logo a span").show();
} else {
    $(".logo a").html("<img class='logo_top' src="+ default_logo + ">");
}
if(scroll_logo != ""){
    $(".logo a").append("<img class='logo_fixed' src="+ scroll_logo + ">");
}
/**
 * 카테고리 마우스 오버 이미지
 * 카테고리 서브 메뉴 출력
    ...
    var methods = {
        ...
        show: function(overNode, iCateNo) {
            ...
            $(methods.aSubCategory[iCateNo]).each(function() {
                ...
                /*aHtml.push('<li><a href="'+this.link_product_list+'">'+this.name+'</a></li>'); 졔 수정-중분류(기획전 링크 연결)*/
            });
            aHtml.push('</ul>');
...
$(function(){
    ...
    $(".btn_close_right").click(function(){
        $(".right_panel").animate({
            "right" : "-420px"
        })
    });
...
$(function(){
	$pop_cnt = 1;
	$pop_length = $(".popupWrap1 .popupView > div").size();
	$(".popupWrap1 .popupView > div").each(function(){
		var $view = $(this).index()+1;
		$(this).addClass("view"+$view);
	});
	function pop_next(){
		$(".popupWrap1 .btnList ul li").eq($pop_cnt - 1).removeClass("on");
		$(".popupWrap1 .popupView > .view"+$pop_cnt).fadeOut();
		$pop_cnt ++;
		if($pop_cnt > $pop_length){
			$pop_cnt = 1;
		}
		$(".popupWrap1 .btnList ul li").eq($pop_cnt - 1).addClass("on");
		$(".popupWrap1 .popupView > .view"+$pop_cnt).fadeIn();
		var $btnL = $(".popupWrap1 .btnList ul li").eq($pop_cnt - 1).offset().left;
		var $btnT = $(".popupWrap1 .btnList ul li").eq($pop_cnt - 1).offset().top;
		var $borderL = $(".popupWrap1 .btnList .border").offset().left;
		var $borderT = $(".popupWrap1 .btnList .border").offset().top;
		$(".popupWrap1 .btnList .border").animate({
			left : "+="+($btnL-$borderL)+"px",
			top : "+="+($btnT-$borderT)+"px"
		}, 200);
	}
	var z = setInterval(pop_next, 2000);
	$(".popupWrap1 .btnList ul li").click(function(){
		var $btn = $(this).index()+1;
		var $btnL = $(this).offset().left;
		var $btnT = $(this).offset().top;
		var $borderL = $(".popupWrap1 .btnList .border").offset().left;
		var $borderT = $(".popupWrap1 .btnList .border").offset().top;
		$(".popupWrap1 .popupView > div").fadeOut();
		$(".popupWrap1 .popupView .view"+$btn).fadeIn();
		$(this).addClass("on");
    ...
    if(popup_display == 1) {
        $("#pop2").addClass("hide");
    } else if(popup_display == 2) {
        $("#pop1").addClass("hide");
    } else if(popup_display == 0) {
        $("#pop1, #pop2").addClass("hide");
    } else {
        console.log("팝업 노출을 설정해주세요.(매뉴얼 참조)");
    }
...
$(function(){
	$p_cnt = 1;
	$p_length = $(".popupWrap2 .popupView > div").size();
	$(".popupWrap2 .popupView > div").each(function(){
		var $viewport = $(this).index()+1;
		$(this).addClass("view"+$viewport);
	});
	function pop_next2(){
		$(".popupWrap2 .btnList ul li").eq($p_cnt - 1).removeClass("on");
		$(".popupWrap2 .popupView > .view"+$p_cnt).fadeOut();
		$p_cnt ++;
		if($p_cnt > $p_length){
			$p_cnt = 1;
		}
		$(".popupWrap2 .btnList ul li").eq($p_cnt - 1).addClass("on");
		$(".popupWrap2 .popupView > .view"+$p_cnt).fadeIn();
        /*
		var $p_btnL = $(".popupWrap2 .btnList ul li").eq($p_cnt - 1).offset().left;
		var $p_btnT = $(".popupWrap2 .btnList ul li").eq($p_cnt - 1).offset().top;
		var $p_borderL = $(".popupWrap2 .btnList .border").offset().left;
		var $p_borderT = $(".popupWrap2 .btnList .border").offset().top;
		$(".popupWrap2 .btnList .border").animate({
			left : "+="+($p_btnL-$p_borderL)+"px",
			top : "+="+($p_btnT-$p_borderT)+"px"
		}, 200);
        */
	}
	var z2 = setInterval(pop_next2, 2000);
	$(".popupWrap2").hover(function(){
		clearInterval(z2);
	}, function(){
		clearInterval(z2);
		z2 = setInterval(pop_next2, 2000);
	});
    
	$(".popupWrap2 .btnList ul li").click(function(){
		var $p_btn = $(this).index()+1;
        /*
		var $p_btnL = $(this).offset().left;
		var $p_btnT = $(this).offset().top;
		var $p_borderL = $(".popupWrap2 .btnList .border").offset().left;
		var $p_borderT = $(".popupWrap2 .btnList .border").offset().top;
		$(".popupWrap2 .btnList .border").animate({
			left : "+="+($p_btnL-$p_borderL)+"px",
        ...
        */
		$(".popupWrap2 .popupView > div").fadeOut();
		$(".popupWrap2 .popupView .view"+$p_btn).fadeIn();
		$(this).addClass("on");
		$(this).siblings().removeClass("on");
		$p_cnt = $p_btn;
	});
