...
var EC_INTRO_FRONT_REDIRECTING = {

    init: function () {

        // 리다이렉트 후에 국기 클릭 또는 레이어창에서 기본몰로 재진입 했을 경우 재리다이렉트 막음
        if (/redirect/.test(location.href) === true || /country/.test(location.href)) return;

        if (aRedirectMultiShopList == null || aRedirectMultiShopList == 'undefined' || aRedirectMultiShopList == '') return;

        if (
            typeof EC_GLOBAL_MALL_LANGUAGE_CODES === "undefined" ||
            typeof EC_GLOBAL_MALL_LANGUAGE_CODES.oStore === "undefined" ||
            typeof EC_GLOBAL_MALL_LANGUAGE_CODES.oStore.oBrowserRedirectLanguage === "undefined"
        ) {
            return;
        }
        ...
        if (navigator.language != null) { // 크롬이나 파폭
            langfull = navigator.language;
        } else if (navigator.userLanguage != null) { // 익스
            langfull = navigator.userLanguage;
        } else if (navigator.systemLanguage != null) { // 그 외
            langfull = navigator.systemLanguage;
        } else { // 이도저도 아니면
            return;
        }

        langfull = langfull.toLowerCase();
        lang = langfull.substring(0, 2);

        var sLang1 = '';
        var sLang2 = '';

        // 크롬 : ko, en-US, ja, (zh-CN, zh-TW), es, pt(포르투갈어는 선택이 안됨)
        // 파이어폭스 : ko-KR, en-US, ja, (zh-CN, zh-TW), es, pt
        // 익스플로러ie : ko-KR, en-US, ja, (zh-CN, zh-TW), es, pt(windows 언어설정에 따라 변경됨)
        // 사파리 : ko-KR, en-US, ja, (zh-CN, zh-TW), es, pt(windows 언어설정에 따라 변경됨)
        // 모바일 사파리(앱) :
        // 모바일 안드로이드 웹 뷰 :

        // 브라우저별 우선순위 : https://wiki.simplexi.com/pages/viewpage.action?pageId=796987652

        var oRedirectInfo = EC_GLOBAL_MALL_LANGUAGE_CODES.oStore.oBrowserRedirectLanguage;

        // 정의된 언어일 경우
        if (oRedirectInfo.hasOwnProperty(lang) === true) {
            // zh-cn, zh-tw
            if (oRedirectInfo[lang].hasOwnProperty('detail') === true) {
                var oDetailRedirectInfo = oRedirectInfo[lang]['detail'];
                var sDetailKey = oDetailRedirectInfo.hasOwnProperty(langfull) === true ? langfull : 'default';

                sLang1 = oDetailRedirectInfo[sDetailKey]['primary'];
                sLang2 = oDetailRedirectInfo[sDetailKey]['secondary'];

            // 그 외 언어
            } else {
                sLang1 = oRedirectInfo[lang]['primary'];
                sLang2 = oRedirectInfo[lang]['secondary'];
            }
        // 정의되지 않은 언어일 경우
        } else {
            sLang1 = oRedirectInfo['default']['primary'];
            sLang2 = oRedirectInfo['default']['secondary'];
        }

        var aRedirectLinkTmp1 = Array();
        var aRedirectLinkTmp2 = Array();

        for (i = 0; i < aRedirectMultiShopList.length; i++) {
            if (aRedirectMultiShopList[i]['language'] == sLang1) {
                // 같은 언어 쇼핑몰이 2개 존재할 경우 shop Number가 빠른 쇼핑몰
                if (aRedirectLinkTmp1 == '') aRedirectLinkTmp1 = aRedirectMultiShopList[i];
            } else if (aRedirectMultiShopList[i]['language'] == sLang2) {
                // 같은 언어 쇼핑몰이 2개 존재할 경우 shop Number가 빠른 쇼핑몰
                if (aRedirectLinkTmp2 == '') aRedirectLinkTmp2 = aRedirectMultiShopList[i];
            }
        }

        var aRedirectLink = Array();

        if (aRedirectLinkTmp1 != '') {
            aRedirectLink = aRedirectLinkTmp1;
        } else if (aRedirectLinkTmp2 != '') {
            aRedirectLink = aRedirectLinkTmp2;
        } else {
            aRedirectLink['shop_no'] = 1;
            aRedirectLink['primary_domain'] = '//' + sMallIdForDomain + '.'.EC_ROOT_DOMAIN;
        }

        // shop_no=1 기본몰이면 리다이렉팅 하지 않음
        if (aRedirectLink['shop_no'] == 1) return;

        // ECQAINT-24890 4차 도메인은 SSL 접속 불가능 하여 http 프로토콜로 리다이렉션 처리
        var sProtocol = '';
        var sMultiShopRegexpPattern = /^(m(obile\-{2})?(\.)?)?shop[0-9]+\./gi;
        if (sMultiShopRegexpPattern.test(aRedirectLink['primary_domain'])) {
            sProtocol = 'http:';
        }

        location.href = sProtocol + '//' + aRedirectLink['primary_domain'];
    }
};

EC$(function() {
    EC_INTRO_FRONT_REDIRECTING.init();
});
